 ...
 *
 * 1. Fiber是什么？
 * Fiber是React16对diff算法做的一次重构，为了充分利用浏览器的空闲时间来做diff处理，
 * 解决动画、输入等高优先级的任务被原来的高层级组件树递归diff算法卡顿。Fiber利用新的任务调度算法，
 * 模拟了浏览器的requestIdleCallback接口功能，
 ...
 * 
 * 5. Hooks如何实现？
 * - useReducer|useState: 把缓存内容更新后将nextUnitOfWork改为root，等待下次workLoop
 * - useEffect: 在commit阶段执行callback，在delete时执行destory
 * - useLayout: 在reconcile阶段执行callback，在delete时执行destory
 * - useMemo|useCallback: 存储值或者回调函数到fiber上，通过created状态标识是否已经调用
 * - useRef: 将RefNode存储在fiber上，永远跟随fiber
 * - useImperativeHandle: 将RefNode.current值修改为给定的新值
 * - forwardRef: 把props.ref字段的值传递给属性内部
 * - createContext: 返回一个Provider高阶组件，该组件的props中存储当前的Context
 * - useContext: 通过fiber -> fiber.return方向找到第一个props中存在的Context，取值即可
 */
